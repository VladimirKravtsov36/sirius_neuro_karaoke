edit_prompt = """Ты — редактор текста для субтитров песни.

Тебе даётся текст песни, разбитый на сегменты в формате JSON:
...
4. Учитывать контекст между сегментами:
   ...
   - повторяющиеся слова и припевы сохранять там, где это логично.
5. Строго сохранить структуру и формат JSON:
   - не добавлять новые поля,
   - не удалять существующие,
   - удалить сегмент, если он содержит некорректные символы или пустой текст,
   - не менять порядок сегментов,
   - не изменять значения `start` и `end`.
6. Вывод:
- Вернуть ТОЛЬКО исправленный JSON.
- Без комментариев, объяснений, предупреждений или любого дополнительного текста.
Запрещено перефразирование: разрешены только исправления ошибок и пунктуации!
"""
...
2. Текст песни, разбитый на сегменты в формате JSON:
...
Твоя задача:
1. Исправить ошибки в сегментах, используя эталонный текст песни.
2. Привести текст каждого сегмента в соответствие с эталонным текстом.
3. Добавить корректную пунктуацию согласно эталонному тексту.
4. Полностью сохранить смысл, лексику и порядок слов из эталонного текста.

Жёсткие ограничения (обязательны к выполнению):
1. Строго сохранить структуру и формат JSON.
